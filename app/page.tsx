        ...
        <div className="flex-1 px-4 py-4 sm:px-7 sm:py-6 lg:px-9">
          ...
          <Tabs defaultValue="airdrop" className="gap-5">
            ...
            <TabsContent value="party" className="mt-0 grid gap-5 xl:grid-cols-[1.5fr_.85fr]">
              <Panel className="p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="label">YOUR SQUAD</p><h2 className="mt-2 text-2xl font-black">ตี้ 1 <span className="text-red-500">/</span> ร้าน A</h2></div><Status>5 / 5 คน</Status></div><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-5">{members.map((member) => <div key={member.name} className="rounded-lg border border-white/10 bg-white/[.025] p-3 text-center"><span className="mx-auto grid h-9 w-9 place-items-center rounded-md bg-slate-800 text-sm font-black text-slate-300">{member.name[0]}</span><p className="mt-2 truncate text-xs font-black">{member.name}</p></div>)}</div><div className="mt-5 flex flex-col gap-4 rounded-lg border border-red-500/20 bg-red-950/20 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-black">PARTY RUNS วันนี้: <span className="text-red-400">{partyCount}</span></p><p className="mt-1 text-xs text-slate-400">รายการที่ผ่าน สมาชิกทั้งตี้ได้คนละ 1 แต้ม</p></div><button onClick={() => setPartyCount((count) => count + 1)} className="red-action !w-full !py-2.5 sm:!w-auto"><Camera className="h-4 w-4" />ส่งกิจกรรมใหม่</button></div></Panel>
              <Panel className="p-5 sm:p-6"><p className="label">PARTY SCOREBOARD</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="score-card"><p>ส่งแล้ว</p><strong>{partyCount}</strong><small>รายการ</small></div><div className="score-card red"><p>รอตรวจ</p><strong>01</strong><small>รายการ</small></div></div><div className="mt-5 space-y-2">{["กิจกรรม #12 · รอตรวจ", "กิจกรรม #11 · ผ่านแล้ว +1", "กิจกรรม #10 · ผ่านแล้ว +1"].map((item) => <div key={item} className="flex items-center justify-between border-b border-white/10 py-3 text-sm font-bold"><span>{item}</span><ChevronRight className="h-4 w-4 text-red-400" /></div>)}</div></Panel>
            </TabsContent>
