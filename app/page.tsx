...
export default function Home() {
  ...
  const [starredMembers, setStarredMembers] = useState<string[]>(["Boss"]);
        ...
        <div className="flex-1 px-4 py-4 sm:px-7 sm:py-6 lg:px-9">
          ...
          <Tabs value={view} onValueChange={(nextView) => setView(nextView as View)} className="gap-5">
            ...
            <TabsContent value="party" className="mt-0 grid gap-5 xl:grid-cols-[1.5fr_.85fr]">
              <Panel className="p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="label">CREATE A PARTY</p><h2 className="mt-2 text-2xl font-black">รวมตี้สำหรับกิจกรรมนี้</h2><p className="mt-2 text-sm text-slate-400">เลือกคนใหม่ได้ทุกครั้ง ตี้นี้จะใช้เฉพาะกิจกรรมที่กำลังส่ง</p></div><Status tone={partyCrew.length === 5 ? "good" : "wait"}>{partyCrew.length} / 5 คน</Status></div><p className="mt-5 flex items-center gap-2 text-xs font-bold text-amber-300"><Star className="h-4 w-4 fill-current" />คนติดดาวจะขึ้นก่อน เพื่อรวมตี้ได้เร็วขึ้น</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">{[...partyCandidates].sort((a, b) => Number(starredMembers.includes(b)) - Number(starredMembers.includes(a))).map((name) => { const chosen = partyCrew.includes(name); const starred = starredMembers.includes(name); return <div key={name} className="relative"><button onClick={() => setPartyCrew((crew) => chosen ? crew.filter((member) => member !== name) : crew.length < 5 ? [...crew, name] : crew)} className={`w-full rounded-lg border p-3 text-center transition ${chosen ? "border-red-500/60 bg-red-600/15 text-white" : "border-white/10 bg-white/[.025] text-slate-400 hover:border-white/30"}`}><span className={`mx-auto grid h-9 w-9 place-items-center rounded-md text-sm font-black ${chosen ? "bg-red-600 text-white" : "bg-slate-800 text-slate-300"}`}>{name[0]}</span><p className="mt-2 truncate text-xs font-black">{name}</p></button><button aria-label={`${starred ? "เอาดาวออกจาก" : "ติดดาว"} ${name}`} onClick={() => setStarredMembers((current) => starred ? current.filter((member) => member !== name) : [...current, name])} className={`absolute right-1.5 top-1.5 grid h-6 w-6 place-items-center rounded-md transition ${starred ? "bg-amber-400 text-black" : "bg-black/40 text-slate-500 hover:text-amber-300"}`}><Star className={`h-3.5 w-3.5 ${starred ? "fill-current" : ""}`} /></button></div>; })}</div><div className="mt-5 flex flex-col gap-4 rounded-lg border border-red-500/20 bg-red-950/20 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-black">PARTY RUNS วันนี้: <span className="text-red-400">{partyCount}</span></p><p className="mt-1 text-xs text-slate-400">ต้องเลือกครบ 5 คน แล้วทุกคนจะได้ +1 เมื่อรายการผ่าน</p></div><button disabled={partyCrew.length !== 5} onClick={() => { setPartyCount((count) => count + 1); setPartyCrew([]); }} className="red-action !w-full !py-2.5 disabled:cursor-not-allowed disabled:opacity-40 sm:!w-auto"><Camera className="h-4 w-4" />สร้างตี้และส่งกิจกรรม</button></div></Panel>
